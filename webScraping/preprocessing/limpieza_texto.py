...
def limpiar_y_normalizar_corpus(
    ...
    # Eliminación de duplicados y reinicio de índice
    filas_iniciales = len(df)
    df = df.drop_duplicates().reset_index(drop=True)
    print(f"Duplicados eliminados. Filas restantes: {len(df)} (Eliminados: {filas_iniciales - len(df)})")
    
    # Eliminación de filas con texto nulo en columnas clave
    filas_con_duplicados = len(df)
    df = df.dropna(subset=['texto', 'texto_comentario']).reset_index(drop=True)
    print(f"Filas nulas eliminadas. Filas restantes: {len(df)} (Eliminados: {filas_con_duplicados - len(df)})")
